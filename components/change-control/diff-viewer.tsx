...
export function DiffViewer({ diffJson, oldVersion, newVersion }: DiffViewerProps) {
    ...
    if (!diffJson) {
        return (
            <div className="flex h-48 items-center justify-center rounded-xl border border-border bg-muted/50">
                <p className="text-sm text-muted-foreground italic">No diff data available for this Change Control.</p>
            </div>
        )
    }
        ...
        <div className="rounded-xl border border-border overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-border bg-muted/50 px-4 py-2">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Document Diff</span>
                </div>
                <div className="flex items-center gap-1 rounded-lg border border-border bg-card p-0.5">
                    <Button
                        size="sm"
                        variant="ghost"
                        className={cn('h-7 text-xs px-3', !showChangesOnly && 'bg-primary text-primary-foreground hover:bg-primary/90')}
            ...
            <div className="grid grid-cols-2 border-b border-border">
                <div className="border-r border-border bg-red-500/10 px-4 py-2">
                    <span className="text-xs font-semibold text-red-600 dark:text-red-500 uppercase tracking-wide">Old — {oldVersion}</span>
                </div>
                <div className="bg-green-500/10 px-4 py-2">
                    <span className="text-xs font-semibold text-green-600 dark:text-green-500 uppercase tracking-wide">New — {newVersion}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 max-h-[500px] overflow-y-auto">
                ...
                <div className="border-r border-border bg-red-500/5">
                    {visibleLeft.map((entry, i) => (
                        <div
                            ...
                            className={cn(
                                ...
                                entry.type === 'removed'
                                    ? 'border-l-red-500/50 bg-red-500/10 text-red-700 dark:text-red-400 line-through'
                    ...
                    {visibleRight.map((entry, i) => (
                        <div
                            ...
                            className={cn(
                                'border-l-2 px-4 py-1.5 text-sm leading-relaxed font-mono',
                                entry.type === 'added'
                                    ? 'border-l-green-500/50 bg-green-500/10 text-green-700 dark:text-green-400'
                                    : 'border-l-transparent text-muted-foreground'
                            )}
                        >
                            {entry.text || '\u00A0'}
